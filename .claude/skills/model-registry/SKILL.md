...
When the proxy detects an image in the request, it forces a vision-capable model. The forced model **must exist on the routed upstream** — different upstreams have different vision-capable models.

| Upstream | Forced vision model | Why |
|----------|---------------------|-----|
...
**Rule:** Never use a single hardcoded vision model. Always select based on `route.upstream`. The selection function lives in `src/index.ts`:

```typescript
function getVisionModel(upstream: string): string {
  if (upstream.includes("/zen/go")) return "qwen3.6-plus";
  if (upstream.includes("/zen")) return "mimo-v2.5-free";
  return "qwen3.6-plus";
}
```
...
6. If the model is a vision-capable replacement, update `getVisionModel()` accordingly
